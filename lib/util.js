...
class LabelMap {
	...
	constructor(labelNames = []) {
		// labelNames is already sorted when passed from Metric classes
		// Store the array directly to avoid unnecessary copying and sorting
		this.#labelNamesArray = labelNames;
	...
	keyFrom(labels = {}) {
		...
		let res = `${labels[names[0]]}`;
		for (let i = 1; i < names.length; i++) {
			const labelName = names[i];
			res += `|${labels[labelName]}`;
		}
