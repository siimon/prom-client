...
class Gauge {
	 ...
	 * @param {BigNumber} value - Value to set the gauge to, must be positive
	 * @param {(Number|Date)} timestamp - Timestamp to set the gauge to
	 * @returns {void}
	 */
	set(labels, value, timestamp) {
		if (typeof labels === 'number' || BigNumber.isBigNumber(labels)) {
	 ...
	 * @param {BigNumber} value - Value to increment - if omitted, increment with 1
	 * @param {(Number|Date)} timestamp - Timestamp to set the gauge to
	 * @returns {void}
	 */
	 ...
	 * @param {object} labels - Object with labels where key is the label key and value is label value. Can only be one level deep
	 * @param {BigNumber} value - Value to decrement - if omitted, decrement with 1
	 * @param {(Number|Date)} timestamp - Timestamp to set the gauge to
	 * @returns {void}
	 */
...
function set(labels) {
	return (value, timestamp) => {
		if (typeof value !== 'number' && !BigNumber.isBigNumber(value)) {
			throw new TypeError(
				`Value is not a valid number: ${util.format(labels)}`
			);
		}
