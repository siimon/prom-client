...
class Counter extends Metric {
	 ...
	 * @param {Number|BigInt} value - Value to increment, if omitted increment with 1
	...
	incWithoutExemplar(labels, value) {
		...
		if (value && !Number.isFinite(value) && typeof value !== 'bigint') {
...
function setValue(hashMap, value, labels = {}, hash = '') {
	if (hashMap[hash]) {
		if (typeof value === 'bigint') {
			hashMap[hash].value = BigInt(hashMap[hash].value) + value;
		} else if (typeof hashMap[hash].value === 'bigint') {
			hashMap[hash].value += BigInt(value);
		} else {
