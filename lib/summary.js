...
class Summary {
	 ...
	 * @param {object} labels - Object with labels where key is the label key and value is label value. Can only be one level deep
	 * @param {Number} factor - Number for multiple the duration time. example multiple by 1e3 to get milliseconds unit
	 ...
	 * });
	 */
	startTimer(labels, factor) {
		return startTimer.call(this, labels, factor)();
	}
...
function startTimer(startLabels, factor) {
	factor = factor || 1;
	return () => {
